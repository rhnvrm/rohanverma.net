...
og_image = "/images/pages/harness-engineering/bubblewrap.png"
+++

![It's Moops!](/images/pages/harness-engineering/bubblewrap.png)

*In Seinfeld's ["The Bubble Boy"](https://en.wikipedia.org/wiki/The_Bubble_Boy), George plays Trivial Pursuit with a kid living in an isolation bubble. The card says "Moors" but is misprinted as "Moops." George, being George, insists on the misprint. The Bubble Boy tries to strangle him but can't get through the bubble. That's bubblewrap in a nutshell: even if the agent is a Terminator hell-bent on strangling your filesystem, it can't get through.*

For the broader sandboxing story (why isolation matters, how it fits with Nix, the Docker comparison), see [the sandbox](@/pages/harness-engineering/sandbox/the-sandbox.md). This page covers the bubblewrap specifics.
...
Bubblewrap is one half of [the sandbox](@/pages/harness-engineering/sandbox/the-sandbox.md). Nix is the other. Bubblewrap says *where* the agent can go. Nix says *what tools* it finds when it gets there.
